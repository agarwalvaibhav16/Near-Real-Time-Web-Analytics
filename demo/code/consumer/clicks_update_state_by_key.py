    ...
    try:
        ...
        return [
            {"time": datetime.strptime(s[5], "%Y-%m-%d %H:%M:%S"),
             ...
             "platform": s[3],"pagename":s[2],"click_event":s[4]}]
...
if __name__ == "__main__":
    ...
    # Parse file
    rdd_orders = filestream.filter(lambda x:"profiled" in x.lower()).flatMap(parse_data)
    ...
    ordersWindow = rdd_orders.map(lambda x: (x['click_event'],1))
    ...
    order_count.saveAsTextFiles("/home/centos/Downloads/final_project/output_dir_clicks/click_count.txt")
